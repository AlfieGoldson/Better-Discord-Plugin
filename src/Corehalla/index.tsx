...
const plugin: PluginFactory = (Plugin, Api) => {
    ...
    return class HideMutedServers extends Plugin {
        ...
        hideGuilds() {
            ...
            const guildListInstance = Utilities.findInTree(
                ReactTools.getReactInstance(guildList),
                (n) => n && n.type && n.type.displayName && n.type.displayName === 'Guilds',
            ...
            this.guildListPatch = Patcher.after(Guilds.prototype, 'render', (thisObject, args, ret) => {
                const guildContainer = Utilities.findInReactTree(
                    ret,
                    (a) => a && a['data-ref-id'] && a['data-ref-id'] === 'guildsnav',
                );
                ...
                for (const guild of guilds) {
                    ...
                    guild.props.guildIds.splice(
                        0,
                        guild.props.guildIds.length,
                        ...guild.props.guildIds.filter((g) => !isGuildMuted(g)),
                    );
                    const unpatchFolder = () => {
